...
<style>
    ...
    .trust-card { background: linear-gradient(90deg, #f0fdf4 0%, #dcfce7 100%); padding: 20px; border-radius: 10px; border-left: 5px solid #16a34a; }
</style>
""", unsafe_allow_html=True)

# ==========================================
# 🌍 VERİ TABANI & MODELLER (Giriş Öncesi Yükleme)
# ==========================================
...
if not global_state or "pools" not in global_state: global_state = {"pools": {"Global-Alpha": {"pool_id": "Global-Alpha", "pool_name": "🌐 Global Alpha Pool", "currency": "USD", "monthly_contribution": 1000, "total_months": 4, "current_month": 1, "total_yield": 0.0, "strategy": "🤖 Smart Yield (Otomatik)", "users": [], "history": []}}}
pools_dict = global_state["pools"]

# ==========================================
# 🔐 ÇİFT YÖNLÜ KİMLİK DOĞRULAMA (LOGIN)
# ==========================================
...
if not st.session_state["authenticated"]:
    ...
    with col2:
        ...
        st.markdown("<p style='text-align: center; color: #6b7280; margin-bottom: 30px;'>Kimlik Doğrulama Ağ Geçidi</p>", unsafe_allow_html=True)
        with st.form("login_form"):
            username = st.text_input("Kullanıcı ID (Adınız veya 'admin')")
            ...
            if st.form_submit_button("Sisteme Giriş Yap", type="primary", use_container_width=True):
                ...
                if username == "admin" and password == "susu2026":
                    ...
                    st.success("✅ Yönetici Paneline Yönlendiriliyorsunuz..."); time.sleep(1); st.rerun()
                # 2. Müşteri (Yatırımcı) Kontrolü
                elif password == "1234":
                    user_found = False
                    for pid, pdata in pools_dict.items():
                    ...
                    if user_found:
                        ...
                        st.success(f"✅ Hoş geldin {username}. Cüzdanın hazırlanıyor..."); time.sleep(1); st.rerun()
                    else:
                        st.error("❌ Kullanıcı bulunamadı. Lütfen fon yöneticinizin sizi ağa eklediğinden emin olun.")
                else: st.error("❌ Hatalı ID veya Güvenlik Anahtarı!")
        st.markdown("</div>", unsafe_allow_html=True)
    st.stop()

# ==========================================
# 📡 CANLI PİYASA VERİSİ & SINIFLAR
# ==========================================
...
def get_live_market_data():
    ...
    try:
        ...
        for symbol, name in assets.items():
            ...
            if len(hist) >= 2: current = float(hist['Close'].iloc[-1]); prev = float(hist['Close'].iloc[0]); rates[name] = {"price": current, "change": (current - prev) / prev}
    except: pass
    return rates
...
if st.session_state["role"] == "admin":
    with st.sidebar:
        ...
        st.caption("WealthTech OS | v17.0 Dual-Portal")
        
        if st.button("🚪 Güvenli Çıkış", use_container_width=True):
            st.session_state["authenticated"] = False; st.session_state["role"] = None; st.session_state["current_user"] = None; st.rerun()
            
        st.divider()
        if not pools_dict: st.error("Havuz yok. Yeni havuz oluşturun.")
        else:
            ...
            current_month = p_data["current_month"]; total_months = p_data["total_months"]
        ...
        with st.form("create_pool_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Yeni Havuzu Başlat"):
                if new_pool_name:
                    ...
                    save_global_state_to_db(global_state); st.success(f"Havuz oluşturuldu!"); time.sleep(1); st.rerun()
    ...
    if pools_dict:
        ...
        with tab1:
            ...
            with col_left:
                ...
                if not active_users: st.warning("Bu havuz boş.")
                else:
                                    ...
                                    if u.has_paid: st.success(f"{u.name}")
                                        ...
                                        with st.popover(f"💳 Ödeme: {u.name[:8]}", use_container_width=True):
                                            ...
                                            with st.form(f"pos_{p_data['pool_id']}_{u.user_id}"):
                                                ...
                                                st.text_input("Kart Numarası", value="4542 1123 9087 4092")
                                                c1, c2 = st.columns(2)
                                                c1.text_input("SKT", value="12/28")
                                                c2.text_input("CVC", value="***", type="password")
                                                if st.form_submit_button("🔒 Çekim Yap", type="primary", use_container_width=True):
                                                    with st.spinner("İşleniyor..."): time.sleep(1)
        ...
        with tab2:
            ...
            else: st.info("Canlı piyasa verileri ile kura çekimi yapılmalıdır.")

# ==========================================
# 🔵 MÜŞTERİ (CLIENT) EKRANI
# ==========================================
    ...
    with st.sidebar:
        ...
        st.markdown(f"### {curr_user}")
        st.caption("Bireysel Yatırımcı Portalı")
        if st.button("🚪 Çıkış Yap", use_container_width=True):
            st.session_state["authenticated"] = False; st.session_state["role"] = None; st.session_state["current_user"] = None; st.rerun()
            
    # Kullanıcının aktif olduğu havuzları ve skorunu hesapla
    user_pools = []
    trust_scores = []
    for pid, pdata in pools_dict.items():
        for u in pdata.get("users", []):
            if u.get("name", "").lower() == curr_user.lower():
                user_pools.append({"pool": pdata, "user_data": u})
                trust_scores.append(u.get("trust_score", 50))
                
    avg_trust = int(sum(trust_scores) / len(trust_scores)) if trust_scores else 50
    
    st.title(f"👋 Hoş Geldin, {curr_user}")
    st.divider()
    
    col_k, col_p = st.columns([1, 2])
    with col_k:
        # ÖZEL İSTEK: GÜVEN KARNEM BAŞLIĞI
        st.subheader("Güven Karnem")
        st.markdown(f"""
        <div class='trust-card'>
            <h1 style='color: #16a34a; margin:0;'>{avg_trust} ⭐</h1>
            <p style='color: #4b5563; font-weight:600;'>Mükemmel Skor</p>
            <p style='font-size: 12px; color: #6b7280;'>Düzenli ödemeleriniz sayesinde Susu Ağı içerisindeki güven puanınız artmaktadır. Puanınız 90'ın üzerine çıktığında VIP Havuzlara giriş hakkı kazanırsınız.</p>
        </div>
        """, unsafe_allow_html=True)
        st.progress(avg_trust / 100)
        
    with col_p:
        # ÖZEL İSTEK: KURA ÇEKİLİŞİ BAŞLIĞI
        st.subheader("🎲 Kura Çekilişi")
        if not user_pools:
            st.info("Şu anda aktif bir havuza dahil değilsiniz.")
        else:
            for item in user_pools:
                pool = item["pool"]
                udataMükemmel bir vizyon! Bu adımla birlikte Susu uygulaman basit bir "yönetici aracı" olmaktan çıkıp, gerçek bir **B2B2C (Business to Business to Consumer)** platformuna dönüşüyor. 

Yani artık sadece sen değil, ağa eklediğin her yatırımcı kendi cüzdanına giriş yapabilecek. Kendi "Güven Puanlarını" (Trust Score), yatırım yaptıkları havuzun anlık durumunu ve kura sıralarını görebilecekler. Bu, kullanıcılarda büyük bir şeffaflık ve kurumsal güven yaratacaktır.

Sisteme **Rol Bazlı Erişim (Role-Based Access Control - RBAC)** ekliyoruz. Artık tek bir kapıdan iki farklı evrene giriş yapılacak.

### 👥 v17.0 - Çoklu Rol ve Yatırımcı Portalı (Client Dashboard)

GitHub'daki `susu_core.py` dosyanın içeriğini aşağıdaki kod ile güncelle. 

**Yeni Giriş Kuralları:**
1. **Yönetici (Sen):** Kullanıcı Adı: `admin` | Şifre: `susu2026`
2. **Yatırımcı (Müşteri):** Kullanıcı Adı: Sisteme eklediğin kişinin tam adı (Örn: *Ali Veli*) | Şifre: `1234` (Tüm müşteriler için varsayılan MVP şifresi).
...
# ==========================================
# 🌍 VERİ TABANI & BAĞLANTILAR (Login öncesi gerekli)
# ==========================================
SUPABASE_URL = st.secrets.get("SUPABASE_URL")
SUPABASE_KEY = st.secrets.get("SUPABASE_KEY")
...
if st.session_state["role"] == "client":
    ...
    with st.sidebar:
        st.image("[https://cdn-icons-png.flaticon.com/512/2953/2953363.png](https://cdn-icons-png.flaticon.com/512/2953/2953363.png)", width=50)
    ...
    with colA:
        ...
        st.metric("Susu Güven Puanın", f"{user_data['trust_score']} ⭐")
        st.caption("Puanın ne kadar yüksekse, erken çekim şansın o kadar artar.")
        st.markdown("</div>", unsafe_allow_html=True)
    with colB:
        st.markdown("<div class='client-card'>", unsafe_allow_html=True)
        pay_status = "✅ Ödendi" if user_data['has_paid'] else "⏳ Ödeme Bekleniyor"
    ...
    with colC:
        st.markdown("<div class='client-card'>", unsafe_allow_html=True)
        win_status = "🏦 Hesabına Geçti" if user_data['has_received'] else "Sırasını Bekliyor"
...
# ==========================================
    ...
    with st.sidebar:
        ...
        st.caption("WealthTech OS | Admin")
    ...
    with st.container(border=True):
        ...
        with mc2: st.metric("Kümülatif Getiri", f"{p_data.get('total_yield', 0):+,.2f} USD")
        with mc3: st.metric("Havuz Kapasitesi", f"{len(active_users)} / {total_months} Kişi")
        with mc4: st.metric("Ortalama Güven", f"{int(sum(u['trust_score'] for u in active_users)/len(active_users)) if active_users else 0} ⭐")
